...
struct GoHandle {
 public:
  ...
  // constructor: This should only be called by Go code, which automatically
  ...
  // move contructor: the moved-from object *will still be destructed*, so
  // increment the ref counter https://stackoverflow.com/a/20589077/925978
  ...
  // so decrement references to the current handle, and increment references to
  // the moved-from handle (see move constructor for details)
  ...
  uint64_t getRawHandle() const { return handle; }
