...
void marshalBinaryParameters(const Parameters& params, std::ostream& stream) {
  lattigo_marshalBinaryParameters(params.getRawHandle(), &writeToStream,
                                  (void*)(&stream));
}

void marshalBinaryBootstrapParameters(const BootstrappingParameters& btp_params,
                                      std::ostream& stream) {
  lattigo_marshalBinaryBootstrapParameters(btp_params.getRawHandle(),
                                           &writeToStream, (void*)(&stream));
}

void marshalBinarySecretKey(const SecretKey& sk, std::ostream& stream) {
  lattigo_marshalBinarySecretKey(sk.getRawHandle(), &writeToStream,
                                 (void*)(&stream));
}

void marshalBinaryPublicKey(const PublicKey& pk, std::ostream& stream) {
  lattigo_marshalBinaryPublicKey(pk.getRawHandle(), &writeToStream,
                                 (void*)(&stream));
}

void marshalBinaryRelinearizationKey(const RelinearizationKey& relinKey,
                                     std::ostream& stream) {
  lattigo_marshalBinaryRelinearizationKey(relinKey.getRawHandle(),
                                          &writeToStream, (void*)(&stream));
}

void marshalBinaryRotationKeys(const RotationKeys& rotKeys,
                               std::ostream& stream) {
  lattigo_marshalBinaryRotationKeys(rotKeys.getRawHandle(), &writeToStream,
                                    (void*)(&stream));
}
...
Parameters unmarshalBinaryParameters(istream& stream) {
  vector<char> buffer(istreambuf_iterator<char>{stream}, {});
  return Parameters(
      lattigo_unmarshalBinaryParameters(buffer.data(), buffer.size()));
}

BootstrappingParameters unmarshalBinaryBootstrapParameters(
    std::istream& stream) {
  vector<char> buffer(istreambuf_iterator<char>{stream}, {});
  return BootstrappingParameters(
      lattigo_unmarshalBinaryBootstrapParameters(buffer.data(), buffer.size()));
}

SecretKey unmarshalBinarySecretKey(istream& stream) {
  vector<char> buffer(istreambuf_iterator<char>{stream}, {});
  return SecretKey(
      lattigo_unmarshalBinarySecretKey(buffer.data(), buffer.size()));
}

PublicKey unmarshalBinaryPublicKey(istream& stream) {
  vector<char> buffer(istreambuf_iterator<char>{stream}, {});
  return PublicKey(
      lattigo_unmarshalBinaryPublicKey(buffer.data(), buffer.size()));
}

RelinearizationKey unmarshalBinaryRelinearizationKey(istream& stream) {
  vector<char> buffer(istreambuf_iterator<char>{stream}, {});
  return RelinearizationKey(
      lattigo_unmarshalBinaryRelinearizationKey(buffer.data(), buffer.size()));
}

RotationKeys unmarshalBinaryRotationKeys(istream& stream) {
  vector<char> buffer(istreambuf_iterator<char>{stream}, {});
  return RotationKeys(
      lattigo_unmarshalBinaryRotationKeys(buffer.data(), buffer.size()));
}
}  // namespace latticpp
