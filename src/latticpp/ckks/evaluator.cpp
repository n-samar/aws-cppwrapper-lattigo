...
Evaluator newEvaluator(const Parameters& params, const EvaluationKey& evakey) {
  return Evaluator(
      lattigo_newEvaluator(params.getRawHandle(), evakey.getRawHandle()));
}

void rotate(const Evaluator& eval, const Ciphertext& ctIn, uint64_t k,
            Ciphertext& ctOut) {
  lattigo_rotate(eval.getRawHandle(), ctIn.getRawHandle(), k,
                 ctOut.getRawHandle());
}

vector<Ciphertext> rotateHoisted(const Evaluator& eval, const Ciphertext& ctIn,
                                 vector<uint64_t> ks) {
...
void multByConst(const Evaluator& eval, const Ciphertext& ctIn, double constant,
                 Ciphertext& ctOut) {
  lattigo_multByConst(eval.getRawHandle(), ctIn.getRawHandle(), constant,
                      ctOut.getRawHandle());
}

void addConst(const Evaluator& eval, const Ciphertext& ctIn, double constant,
              Ciphertext& ctOut) {
  lattigo_addConst(eval.getRawHandle(), ctIn.getRawHandle(), constant,
                   ctOut.getRawHandle());
}

void rescale(const Evaluator& eval, const Ciphertext& ctIn, double threshold,
             Ciphertext& ctOut) {
  lattigo_rescale(eval.getRawHandle(), ctIn.getRawHandle(), threshold,
                  ctOut.getRawHandle());
}
...
void rescaleMany(const Evaluator& eval, const Parameters& params,
                 ...
                 Ciphertext& ctOut) {
  lattigo_rescaleMany(eval.getRawHandle(), params.getRawHandle(),
                      ctIn.getRawHandle(), numRescales, ctOut.getRawHandle());
}
...
void mulRelin(const Evaluator& eval, const Ciphertext& ct0,
  ...
  lattigo_mulRelin(eval.getRawHandle(), ct0.getRawHandle(), ct1.getRawHandle(),
                   ctOut.getRawHandle());
}

void mul(const Evaluator& eval, const Ciphertext& ct0, const Ciphertext& ct1,
         Ciphertext& ctOut) {
  lattigo_mul(eval.getRawHandle(), ct0.getRawHandle(), ct1.getRawHandle(),
              ctOut.getRawHandle());
}

void mulPlain(const Evaluator& eval, const Ciphertext& ctIn,
              const Plaintext& pt, Ciphertext& ctOut) {
  lattigo_mulPlain(eval.getRawHandle(), ctIn.getRawHandle(), pt.getRawHandle(),
                   ctOut.getRawHandle());
}

void add(const Evaluator& eval, const Ciphertext& ct0, const Ciphertext& ct1,
         Ciphertext& ctOut) {
  lattigo_add(eval.getRawHandle(), ct0.getRawHandle(), ct1.getRawHandle(),
              ctOut.getRawHandle());
}

void addPlain(const Evaluator& eval, const Ciphertext& ctIn,
              const Plaintext& pt, Ciphertext& ctOut) {
  lattigo_addPlain(eval.getRawHandle(), ctIn.getRawHandle(), pt.getRawHandle(),
                   ctOut.getRawHandle());
}
...
void sub(const Evaluator& eval, const Ciphertext& ct0, const Ciphertext& ct1,
         Ciphertext& ctOut) {
  lattigo_sub(eval.getRawHandle(), ct0.getRawHandle(), ct1.getRawHandle(),
              ctOut.getRawHandle());
}

void subPlain(const Evaluator& eval, const Ciphertext& ctIn,
              const Plaintext& pt, Ciphertext& ctOut) {
  lattigo_subPlain(eval.getRawHandle(), ctIn.getRawHandle(), pt.getRawHandle(),
                   ctOut.getRawHandle());
}
...
void multByGaussianIntegerAndAdd(const Evaluator& eval, const Ciphertext& ctIn,
                                 ...
                                 Ciphertext& ctOut) {
  lattigo_multByGaussianIntegerAndAdd(eval.getRawHandle(), ctIn.getRawHandle(),
                                      cReal, cImag, ctOut.getRawHandle());
}
...
void setScale(const Evaluator& eval, const Ciphertext& ct, double scale) {
  lattigo_setScale(eval.getRawHandle(), ct.getRawHandle(), scale);
}
